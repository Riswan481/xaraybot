...
try {
  ...
  console.log('Terjadi kesalahan saat menyimpan session: ' + err);
}
...
async function startsPairing(sock) {
  const credsFile = path.join(sesiPath, 'creds.json');
  if (!fs.existsSync(credsFile)) {
    if (!process.stdin.isTTY) {
      console.log("❌ Pairing tidak bisa dilakukan melalui PM2 / background process.");
      return;
    }
